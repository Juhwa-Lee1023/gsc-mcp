...
import { listAvailablePromptNames } from "../../domain/surface.js";
...
export function registerPrompts(server: McpServer, context: RuntimeContext): void {
  ...
  if (availablePrompts.has("gsc-compare-periods")) {
    server.registerPrompt(
      "gsc-compare-periods",
      {
  ...
  if (availablePrompts.has("gsc-debug-url")) {
    server.registerPrompt(
      "gsc-debug-url",
      {
  ...
  if (availablePrompts.has("gsc-sitemap-audit")) {
    server.registerPrompt(
      "gsc-sitemap-audit",
      {
